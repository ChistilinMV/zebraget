...
Вот популярные варианты, если на вашем сервере Linux используется менеджер процессов:

Если используется PM2 (самый частый вариант):

```bash
pm2 restart all
# или если вы знаете имя процесса, например "zebraget-web":
pm2 restart zebraget-web
```

Если сервер работает как служба systemd (системный сервис):

```bash
sudo systemctl restart имя_вашего_сервиса
# например: sudo systemctl restart zebraget
```

Если сервер запущен для тестов вручную в консоли: Ctrl + C и npm start.

## Пароли по умолчанию

Пароли по умолчанию прописаны в users.json. Если users.json отсутствует, то создаётся пустая база с паролями для admin и superadmin по умолчанию.